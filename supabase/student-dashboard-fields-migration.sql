...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'internship_placements'
      and column_name = 'google_maps_url'
  ) then
    execute 'update public.internship_placements set maps_url = google_maps_url where maps_url is null and google_maps_url is not null';
  end if;
  ...
  if exists (
    ...
    execute 'update public.accommodations set maps_url = google_maps_url where maps_url is null and google_maps_url is not null';
  end if;

  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public'
      and table_name = 'hospital_medical_centers'
      and column_name = 'google_maps_url'
  ) then
    execute 'update public.hospital_medical_centers set maps_url = google_maps_url where maps_url is null and google_maps_url is not null';
  end if;
end $$;
